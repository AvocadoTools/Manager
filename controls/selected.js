...
export default class AvocadoSelected extends HTMLElement {
  constructor() {
    super();
    ...
    this.$slot.addEventListener( 'slotchange', () => this.options = this.children );
  }
  ...
  set options( items ) {
    ...
    for( let c = 0; c < this.$select.children.length; c++ ) {
      this.$select.children[c].disabled = items[c].disabled;      
      this.$select.children[c].selected = items[c].selected;
      this.$select.children[c].value = items[c].value;
      this.$select.children[c].innerText = items[c].innerText;
    }    
  }
...
window.customElements.define( 'adc-selected', AvocadoSelected );
